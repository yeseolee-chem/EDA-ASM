"""SPEC_05 T4-T5 - 2x2 XGB ablation on v7 776-rxn cohort.
...
def load_bundle():
    ...
    rids = b["reaction_ids"]
    X24 = b["descriptors"].numpy()  # (776, 24) m3
    Y = b["labels"].numpy()          # (776, 5)
...
def main():
    ...
    print(f"[m3 v7] N={len(rids)}, D_base=24")
    ...
    ax.set_title(f"SPEC_05 - XGB 2x2 ablation (m3 v7, 776 rxns)\n"
    ...
    # ============ REPORT ============
    lines = ["# SPEC_05 - d25 + soft sum-consistency (XGB) on 776 v7", "",
             f"- Cohort: {len(rids)} rxns (v7 m3 bundle)",
