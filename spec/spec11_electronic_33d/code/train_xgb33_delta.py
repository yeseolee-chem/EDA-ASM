"""SPEC_11 - 2-step learner: 33-d per-channel XGB base (b) + MACE CA residual (delta).

Trimmed clone of spec06/code/train_xgb28_delta.py; the ONLY differences are:
  (a) X = build_X33(rids, X24)  instead of build_X28
  (b) Fold JSON now emits BOTH arm-1 (b_val, i.e. xgb_33d base) and arm-2
      (b_val + delta = xgb33 + delta) predictions -> aggregate_2arm can build
      both arms without recomputing xgb_33d.
...
Idempotent: skip if member{M}.json already exists.
Cross-fit gate #3: abort if median|r_train| < 0.05 * median|y - ybar|.
"""
...
from descriptors33 import build_X33  # noqa: E402
...
def train_one_cell(bundle_full, X, Y, rids, fold_idx, tr_idx, te_idx,
    ...
    print(f"[xgb33+delta] fold{fold_idx} member{member} seed={seed}", flush=True)
...
def main():
    ...
    print(f"[SPEC_11] fold={args.fold}  member={args.member}  device={args.device}",
          flush=True)
    ...
    print(f"  X33 shape={X33.shape}  proxies_ok={n_ok}  zero_fill={n_zero}",
          flush=True)
    ...
    result = {
        ...
        **{f"y_pred_{c}": [float(v) for v in yp[:, i]] for i, c in enumerate(CHANNELS)},
        # arm-1 (base only, xgb_33d): b_val per channel
