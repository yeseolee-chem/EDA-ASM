"""5-fold CV × M-ensemble training for the RTSP heads (B0_RTSP / M1_RTSP).

Same protocol as ``train_cv.py`` (Plan §5), but the per-reaction input is a
(R, TS, P) triple and the bundle/model factories live in ``training_rtsp``.
"""
...
from eda_asm.asr_v1.models_rtsp import BaselineB0RTSP, ModelM1RTSP
from eda_asm.asr_v1.training_rtsp import (
    CachedFeatureBundleRTSP,
    TrainConfigRTSP,
    kfold_indices,
    train_one_model_rtsp,
)


def _build_factory(model_name: str, cfg: dict):
    if model_name == "b0_rtsp":
        c = cfg["baseline_b0"]
        return lambda F: BaselineB0RTSP(
            feature_dim=F, d_hidden=c["d_hidden"],
            head_hidden=c["head_hidden"], dropout=c["dropout"],
        )
    if model_name == "m1_rtsp":
        c = cfg["model_m1"]
        return lambda F: ModelM1RTSP(
            feature_dim=F, d_model=c["d_model"], n_heads=c["n_heads"],
            head_hidden=c["head_hidden"], dropout=c["dropout"],
        )
    raise ValueError(f"unknown RTSP model: {model_name}")
...
def main() -> None:
    ...
    ap.add_argument("--model", choices=["b0_rtsp", "m1_rtsp"], required=True)
    ...
    print(f"[train_cv_rtsp] model={args.model}  output={out_root}")
    print(f"[train_cv_rtsp] features={feature_cache_path}")
    bundle = CachedFeatureBundleRTSP.load(feature_cache_path)
    n = len(bundle)
    print(f"[train_cv_rtsp] loaded {n} reactions, feature_dim={bundle.feature_dim}")

    tcfg = TrainConfigRTSP(
    ...
    print(f"[train_cv_rtsp] device={tcfg.device}  epochs={tcfg.epochs}  "
    ...
    for fold_i, (train_idx, val_idx) in enumerate(splits):
        ...
        for m_idx in range(M):
            ...
            t0 = time.time()
            model, res = train_one_model_rtsp(
            ...
            with torch.no_grad():
                ...
                for j in val_idx:
                    ...
                    preds_this.append(model(r, rm, t, tm, p, pm)[0].cpu())
    ...
    print(f"\n[train_cv_rtsp] DONE in {elapsed:.1f}s   "
          f"→ overall MAE = {summary['overall_mae_kcal_mean']:.3f} "
          f"± {summary['overall_mae_kcal_std']:.3f} kcal/mol")
    print("[train_cv_rtsp] per-component MAE (mean ± std over folds):")
