"""Learning curve for the RTSP heads (B0_RTSP / M1_RTSP).

Sweeps train-fold subset sizes against the same 5-fold × 5-ensemble protocol
as the 2-way ``learning_curve.py``, but feeds (R, TS, P) triples.
"""
...
def main() -> None:
    ...
    print(f"[lc_rtsp] features={feature_cache_path}")
    ...
    print(f"[lc_rtsp] model={args.model}  N={n}  train-fold={train_fold_size}")
    print(f"[lc_rtsp] sizes (requested → capped): {raw_sizes} → {sizes}")
            ...
            for m_idx in range(M):
                ...
                with torch.no_grad():
                    ...
                    for j in val_idx:
                        ...
                        preds.append(model(r, rm, t, tm, p, pm)[0].cpu())
    ...
    print(f"\n[lc_rtsp] DONE in {elapsed:.1f}s — saved {out_root/'summary.json'}")
    print("[lc_rtsp] Learning curve (overall MAE kcal/mol):")
    for r in curve:
