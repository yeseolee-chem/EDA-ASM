#!/bin/bash
# Runs ORCA for the 18 non-OOD user-saved rxns (54 tasks: 18 EDA + 36 SP).
# Single serial job. Manifest: manifest_saved_leftover.txt

#SBATCH --job-name=saved_lo
#SBATCH --partition=cpu2
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --time=48:00:00
#SBATCH --output=/gpfs/tmp_cpu2/yeseo1ee/eda_asm_features/spec_v1_logs/saved_lo.%j.out
#SBATCH --error=/gpfs/tmp_cpu2/yeseo1ee/eda_asm_features/spec_v1_logs/saved_lo.%j.err
...
MANIFEST="$REPO/outputs/v8_review/manifest_saved_leftover.txt"
TOTAL=$(wc -l < "$MANIFEST")
echo "[$(date +%H:%M:%S)] saved_leftover total=$TOTAL node=$(hostname -s)"
