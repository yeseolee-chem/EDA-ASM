...
# TensorFlow imports (OPTIONAL — NN branch broken in Keras 3, sklearn-only path)
...
try:
    ...
    print(f'[hyp_tuning] TF import failed ({_tf_err.__class__.__name__}) — sklearn-only mode')
    _TF_AVAILABLE = False
    tf = None
    kt = None
...
def main():
    ...
    try:    
        ...
        if _TF_AVAILABLE:
            ...
            logger.info('TF unavailable — skipping NN tuning (sklearn only).')
            General._save([st_tuned_dict])
