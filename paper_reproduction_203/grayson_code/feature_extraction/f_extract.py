    ...
    def _load_files(job_type):
        ...
        for file in files:
            if setup['dataset'] == 'ma':
                ...
                rxn_number = file.split('_')[1].replace('.out', '').replace('.log', '')
    ...
    def _load(files, job_type, path):
        ...
        for file in files:
            if job_type == 'gs' or job_type == 'dist_gs':
                ...
                filename = file
                number = file.split('_')[1].replace('.out', '').replace('.log', '')
            ...
            data_dict[file]['path'] = os.path.join(path, file)                
    ...
    def _clean_dictionary(d):
        ...
        for structure in d:
            if 'reopt' in structure:
                ...
                name = structure
            if '.out' in structure:
                new_filename = name.split('_')[1].replace('.out','').replace('.log','')
            else:
    ...
    def _load(files, job_type, path):
        ...
        for file in files:
            if job_type == 'gs':
                ...
                filename = file
                number = file.split('_')[2].replace('.out', '').replace('.log', '')
            d[file] = cclib.io.ccread(file)
            data_dict[file] = {}
            data_dict[file]['reaction_number'] = number
            data_dict[file]['reactant'] = filename
