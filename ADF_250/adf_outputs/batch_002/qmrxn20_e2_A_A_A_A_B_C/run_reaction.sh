#!/bin/bash
# ASR ADF run for reaction qmrxn20_e2_A_A_A_A_B_C
...
#SBATCH --job-name=asr_qmrxn20_e2_A_A_A_A_B_C
#SBATCH --ntasks=1
...
echo "=== qmrxn20_e2_A_A_A_A_B_C START $START_UTC host=$(hostname) NSCM=$NSCM ==="
...
echo "=== qmrxn20_e2_A_A_A_A_B_C END $END_UTC ==="
...
python /gpfs/home1/yeseo1ee/projects/eda-asm-prediction/scripts/adf/write_status.py \
    ...
    --frag-method "smarts" \
    --atoms-a '[2]' \
    --atoms-b '[0, 1, 3, 4, 5, 6, 7, 8]' \
    --charge-a -1 \
    --charge-b 0 \
    --mult-a 1 \
    --mult-b 1 \
    --total-charge -1 \
    --dataset-delta-Ea 31.994987999864087 \
    --atom-permutation '[2, 0, 1, 3, 4, 5, 6, 7, 8]' \
    --single-atom-a 1 \
    --single-atom-b 0 \
...
echo "=== qmrxn20_e2_A_A_A_A_B_C cleanup done ==="
