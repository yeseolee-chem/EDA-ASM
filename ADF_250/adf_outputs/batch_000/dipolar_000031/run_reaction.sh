#!/bin/bash
# ASR ADF run for reaction dipolar_000031
...
#SBATCH --job-name=asr_dipolar_000031
#SBATCH --ntasks=1
...
echo "=== dipolar_000031 START $START_UTC host=$(hostname) NSCM=$NSCM ==="
...
echo "=== dipolar_000031 END $END_UTC ==="
...
python /gpfs/home1/yeseo1ee/projects/eda-asm-prediction/scripts/adf/write_status.py \
    ...
    --basis "TZ2P" \
    --frag-method "smarts" \
    --atoms-a '[0, 1, 2, 3, 4, 5, 6, 14, 15, 16]' \
    --atoms-b '[7, 8, 9, 10, 11, 12, 13, 17, 18, 19]' \
    --charge-a 0 \
    --charge-b 0 \
    --mult-a 1 \
    --mult-b 1 \
    --total-charge 0 \
    --dataset-delta-Ea 20.551995826465863 \
    --atom-permutation '[0, 1, 2, 3, 4, 5, 6, 14, 15, 16, 7, 8, 9, 10, 11, 12, 13, 17, 18, 19]' \
    --single-atom-a 0 \
    --single-atom-b 0 \
...
echo "=== dipolar_000031 cleanup done ==="
