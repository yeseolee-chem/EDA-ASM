#!/bin/bash
# ASR ADF run for reaction dipolar_000114
...
#SBATCH --job-name=asr_dipolar_000114
#SBATCH --ntasks=1
...
echo "=== dipolar_000114 START $START_UTC host=$(hostname) NSCM=$NSCM ==="
...
echo "=== dipolar_000114 END $END_UTC ==="
...
python /gpfs/home1/yeseo1ee/projects/eda-asm-prediction/scripts/adf/write_status.py \
    ...
    --basis "TZ2P" \
    --frag-method "smarts" \
    --atoms-a '[0, 1, 2, 3, 4, 5, 6, 7, 8, 17, 18, 19, 20, 21, 22, 23]' \
    --atoms-b '[9, 10, 11, 12, 13, 14, 15, 16, 24, 25, 26, 27, 28]' \
    --charge-a 0 \
    --charge-b 0 \
    --mult-a 1 \
    --mult-b 1 \
    --total-charge 0 \
    --dataset-delta-Ea 8.543462446100978 \
    --atom-permutation '[0, 1, 2, 3, 4, 5, 6, 7, 8, 17, 18, 19, 20, 21, 22, 23, 9, 10, 11, 12, 13, 14, 15, 16, 24, 25, 26, 27, 28]' \
    --single-atom-a 0 \
    --single-atom-b 0 \
...
echo "=== dipolar_000114 cleanup done ==="
