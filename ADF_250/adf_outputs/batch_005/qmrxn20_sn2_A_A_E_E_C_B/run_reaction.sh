#!/bin/bash
# ASR ADF run for reaction qmrxn20_sn2_A_A_E_E_C_B
...
#SBATCH --job-name=asr_qmrxn20_sn2_A_A_E_E_C_B
#SBATCH --ntasks=1
...
echo "=== qmrxn20_sn2_A_A_E_E_C_B START $START_UTC host=$(hostname) NSCM=$NSCM ==="
...
echo "=== qmrxn20_sn2_A_A_E_E_C_B END $END_UTC ==="
...
python /gpfs/home1/yeseo1ee/projects/eda-asm-prediction/scripts/adf/write_status.py \
    ...
    --frag-method "smarts" \
    --atoms-a '[3]' \
    --atoms-b '[0, 1, 2, 4, 5, 6, 7, 8, 9, 10, 11, 12]' \
    --charge-a -1 \
    --charge-b 0 \
    --mult-a 1 \
    --mult-b 1 \
    --total-charge -1 \
    --dataset-delta-Ea 17.674502415881967 \
    --atom-permutation '[3, 0, 1, 2, 4, 5, 6, 7, 8, 9, 10, 11, 12]' \
    --single-atom-a 1 \
    --single-atom-b 0 \
...
echo "=== qmrxn20_sn2_A_A_E_E_C_B cleanup done ==="
