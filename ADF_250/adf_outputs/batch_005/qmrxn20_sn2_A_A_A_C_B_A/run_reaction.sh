#!/bin/bash
# ASR ADF run for reaction qmrxn20_sn2_A_A_A_C_B_A
...
#SBATCH --job-name=asr_qmrxn20_sn2_A_A_A_C_B_A
#SBATCH --ntasks=1
...
echo "=== qmrxn20_sn2_A_A_A_C_B_A START $START_UTC host=$(hostname) NSCM=$NSCM ==="
...
echo "=== qmrxn20_sn2_A_A_A_C_B_A END $END_UTC ==="
...
python /gpfs/home1/yeseo1ee/projects/eda-asm-prediction/scripts/adf/write_status.py \
    ...
    --frag-method "smarts" \
    --atoms-a '[3]' \
    --atoms-b '[0, 1, 2, 4, 5, 6, 7, 8, 9]' \
    --charge-a -1 \
    --charge-b 0 \
    --mult-a 1 \
    --mult-b 1 \
    --total-charge -1 \
    --dataset-delta-Ea 6.45059971910984 \
    --atom-permutation '[3, 0, 1, 2, 4, 5, 6, 7, 8, 9]' \
    --single-atom-a 1 \
    --single-atom-b 0 \
...
echo "=== qmrxn20_sn2_A_A_A_C_B_A cleanup done ==="
