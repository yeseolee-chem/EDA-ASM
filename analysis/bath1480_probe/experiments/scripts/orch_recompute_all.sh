...
# --- XGB Phase 2/3/5 (15 tasks) — wait until sklearn drains enough ---
wait_for_slot 15
JXGB=$(sbatch --parsable $SCRIPTS/sbatch_xgb_phase235.sh)
echo "XGB Phase 2/3/5 jid=$JXGB $(date -Is)"

# --- Phase 4b physics_24 rebuild (needs disp_xtb removal) ---
# Wait for cohort_v1 free, then run — no HPC time for this small step.
wait_for_slot 1
...
# --- Phase 4c 25 cells, dispatch continuously ---
echo "Phase 4c dispatch loop $(date -Is)"
declare -a SUBMITTED4C
for CELL in $(seq 0 24); do
    FOLD=$((CELL / 5)); SEED=$((CELL % 5))
    if [ -f "$EXP/results/phase4_stacked_xgb_mace/fold_$FOLD/seed_$SEED/metrics.json" ]; then
        echo "SKIP cell $CELL (already done)"
        continue
    fi
    wait_for_slot 1
